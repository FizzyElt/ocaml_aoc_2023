...
let rec find_target_step2 steps count node_map node =
  if String.ends_with ~suffix:"Z" node then count
  else
    let next_node, count =
      List.fold_left
        (fun (next, count) step ->
          match next with
          | Some n ->
              if String.ends_with ~suffix:"Z" n then (None, count)
              else (next_node step n node_map, count + 1)
          | None -> (None, count))
        (Some node, count) steps
    in
    next_node
    |> Option.map (fun n -> find_target_step2 steps count node_map n)
...
let part2_solution () =
  ...
  let start_nodes =
    ...
    |> Seq.map (fun (k, _) -> k)
    |> Seq.map (fun k -> find_target_step2 steps 0 node_map k)
    |> List.of_seq
  in
